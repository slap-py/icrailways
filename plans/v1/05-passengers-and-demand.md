...
- **First playable and validation (09):** now written, and **milestone 3 is the first playable** — the point at which this plan's behaviour becomes the game. It requires purposes, generalised cost over published times, hard capacity, denied boarding, fares and the traveller inspector. The scenario is designed so that at least one transfer is genuinely useful: the express skips a station the regional serves, because two overlapping end-to-end routes would never exercise transfer penalties, re-planning or missed connections.
