...
This document records determinations made using the [game vision](00-game-vision.md), the [development plan roadmap](01-development-plan-roadmap.md), and the preceding subsystem plans [02](02-infrastructure-and-operations.md), [03](03-routes-and-timetabling.md), [04](04-fleet-and-depots.md), and [05](05-passengers-and-demand.md).
...
Success means a player can see why they are or are not solvent, attribute it to specific decisions — this corridor was built to 250 km/h for demand that needed 160, this fleet is larger than the schedules require, this route runs at a fare below what it costs to operate — and that building the right railway is meaningfully cheaper than building an impressive one.
...
The balance moves as events occur, so a player watching a route run sees revenue arrive and running costs accrue. Alongside it, a **statement closes each timetable week** and reports by category: revenue by route, operating cost by category, maintenance, contract subsidy, interest, and the net result.
...
**Nothing is ever blocked for want of money.** Construction, purchases and operations all continue at any balance, however negative. There is no insolvency state, no game over, and no automatic withdrawal of routes.
...
This replaces what this plan originally said, which was that insolvency blocked new construction, purchases and borrowing while operations continued — shaped deliberately after plan 02's rule on infrastructure edits and plan 04's on route overruns, blocking the next commitment but never breaking what was underway. The player removed it. The argument for removal is that a block is a failure state wearing a different hat: the audit had already shown that a zero-revenue start and a structurally loss-making network could both make the block unrecoverable, and removing it dissolves that trap rather than tuning it.
...
### Fares: one national rate, per-route modifiers

The player sets a **base fare rate per passenger-kilometre**, and each route carries a **modifier** for its market position — a premium for an intercity product, a discount for a stopping route chasing volume.

One global lever keeps a large network from becoming a table of numbers to maintain, while the modifier gives the local control that makes a route a commercial decision. A journey's fare is derived from its distance and the modifiers of the services it uses, so a journey across two routes prices coherently without a separate cross-network rule.
...
This is authentically how Swedish regional rail is actually procured, and it does several things at once that a milestone list would not. It gives concrete goals tied to where the player has actually built, rather than generic targets. It creates real tension, because a contracted route must run at hours and frequencies the player would not choose commercially. And it is a second revenue stream, so a corridor that cannot pay for itself on fares alone can still be worth operating — which is the actual economics of regional rail and a genuinely interesting decision.

A contract specifies at minimum a served corridor or place pair, a minimum service level expressed in departures and hours of operation, a term in weeks, and a subsidy per week. Compliance is measured against the published timetable and against actual operation, so a published route that persistently fails to run does not satisfy a contract.
...
**Sandbox does not track total money at all.** There is no balance, no opening capital, no debt, no accumulating total, and no contracts. What it keeps is **ticket price and profitability**: the player still sets fares, and still sees whether a route or a corridor earns more than it costs to run. That is the information a sandbox is most useful for — a player testing whether a corridor would pay needs to see whether it would pay — and it needs no running total to provide it.
...
4. Set the base fare rate and per-route modifiers, and see the forecast effect on journeys and revenue from plan 05.
...
8. Attribute a poor result to a specific cause — overbuilt speed, oversized fleet, a fare below operating cost, an uncontracted thin route — and respond.
...
- **Fare policy:** the base rate per passenger-kilometre and a per-route modifier, with a rule for pricing a journey spanning several routes.
...
- **Interfaces consumed:** built infrastructure and its properties from plan 02; operated trips, published routes, and the timetable week from plan 03; train purchase categories, owned trains, and operated distance from plan 04; travelled journeys and fare response from plan 05.
...
- **Fare modifier bounds:** how far a route modifier may deviate, and whether a contracted route's fare is the player's to set at all.
...
- **Routes and timetabling (03):** supplies the published timetable, operated trips, and the week boundary the statement follows. Contract compliance is measured against the published timetable and actual operation.
...
- **Interface and player experience (07):** now written. The weekly statement, contract browser and attribution of a poor result live in a near-full-screen Finance workspace, together with plan 05's demand diagnostics. Cost previews extend the existing construction sidebar, adding the recurring commitment beside the one-off price with no confirmation step. There are no borrowing controls to design. **Because sandbox tracks no total money, the Finance workspace is absent there rather than empty**, and the bottom bar carries three buttons instead of four — where sandbox's demand diagnostics then live is an open question there.
...
- **First playable and validation (09):** now written. **Fares arrive at milestone 3**, the first playable, so revenue is comparable between weeks before the rest of this plan exists. The ledger, weekly statement, train purchase, one contract, servicing cost and the negative balance all land at milestone 4, whose exit evidence is a complete week closing with a reconcilable ledger that a player can act on.
...
| A week of operation closes | A statement reports revenue by route, operating and maintenance costs by category, subsidy, interest, and the net result. |
...
| Player sets a premium modifier on an intercity route | That route's fares rise without affecting a stopping route on the same corridor. |
| A journey uses two routes with different modifiers | It prices coherently from distance and the modifiers involved, with no separate cross-network rule. |
...
| Player operates in sandbox | Fares, costs and per-route profitability are computed and shown. There is no balance to read, no debt, and no contracts, and nothing is ever blocked. |
| Player asks how much money they have in sandbox | The question has no answer, because no total is kept. Profitability answers whether a route pays, which is the question sandbox exists to serve. |
