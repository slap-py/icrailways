...
Status: agreed direction from the fleet and depots planning discussion, not an implementation specification. The catalogue shape, storage model, turnaround model, servicing trigger, empty-movement ownership, and schedule assignment are settled in kind. Siding capacity and the servicing-only empty movement were settled on 21 September, resolving two findings of that day's audit. The numerical catalogue is proposed and must be verified and balanced before implementation.

This document records determinations made using the [game vision](00-game-vision.md), the [development plan roadmap](01-development-plan-roadmap.md), the [infrastructure and operations plan](02-infrastructure-and-operations.md), and the [routes and timetabling plan](03-routes-and-timetabling.md).
...
Let players buy trains, stable and service them at depots they have built, and assign each schedule to a physical train that can actually work it.

Success means a player can look at a schedule and understand why it is or is not workable — this train is too long for that loop, this electric unit cannot reach that unwired depot, this chain leaves four minutes where the type needs six, this train runs out of service interval on Thursday — and can fix it by buying a train, rebuilding a depot, or re-chaining the schedule.
...
- Assign individual trains to schedules: sequences of trips worked by one physical train.
...
**Coupling** is the only formation control. Two or three units of the *same* type may be coupled into one train, which then works a schedule as a single train: its length is the sum, its capacity is the sum, and its performance figures are those of the single unit. Each type declares the maximum number of units that may be coupled.

Coupling is decided per schedule and holds for the whole schedule. A train does not couple or split partway through a schedule, and units of different types never couple. Dynamic coupling and splitting — joining two portions at a junction station, dividing a train to serve two branches — is deferred.
...
- **Plan 02** consumes length, maximum speed, acceleration, and route braking rate for the piecewise speed profile and the technical minimum running time, and traction for the strict electrification check. Braking rate also feeds the moving-block separation distance.
...
Plan 03's schedule validation checks each consecutive pair of trips in a schedule against it, and reports a shortfall as a named quantity — *this chain leaves four minutes, the X55 needs five*. A turnaround gap is not a dwell and is not authored per stop; it is a floor the chain must clear.
...
A reversal occupies the platform or track the train stands on for its whole duration, per plan 02's occupancy rules. A short turnaround at a single-platform terminus is therefore a capacity problem as well as a schedule problem, and validation reports both.

### Empty movements are derived by the schedule

When a schedule's chain requires a train to move without working a trip — out of a depot to its first trip, off the network at the end of a schedule, or between two trips that do not meet end to end — the game creates an **empty movement** owned by that schedule.
...
The player does not author empty movements from scratch, but they are visible and adjustable: the player can retime one within the gap available, or reject the chain and build a different schedule. This does not breach plan 03's no-automatic-generation rule, which concerns revenue trips and the timetable the player publishes. An empty movement is the mechanical consequence of a chain the player authored, in the same way a derived station time is the consequence of an authored allowance.
...
An electric train assigned to a schedule that stables at an unwired depot is a validation error reported before publication, not a discovery at runtime. This is the case plan 02's acceptance scenarios already name.

### Servicing is triggered by distance

Each type declares a **service interval** in kilometres and a **service duration** in hours. A train accumulates distance from every movement it makes, revenue trips and empty movements alike. When accumulated distance since its last route reaches the interval, the train requires a servicing visit: an unbroken stay of the service duration on a siding at any depot it can reach.

Distance rather than calendar days or running hours, because distance is what makes a long intercity schedule genuinely more demanding than a short local one. Days would make an idle train as urgent as a hard-worked one; hours would penalise slow stopping work hardest, which is backwards.

Servicing is a **scheduling problem, not a failure mode**. A train does not break down and there is no reliability simulation. Plan 03's validation reports, over the published timetable, which trains reach their interval and whether their schedule contains a stay long enough to route them.
...
The exception is an escape hatch, not the intended path. Plan 03's validation reports over the published timetable which trains reach their interval and where, so a player who reads the warning schedules the visit into the schedule and never needs it.
...
A train is an individual object with an identity, a type, an accumulated distance, and a current location. Two X61s are distinguishable, because a schedule is assigned to one of them and only one of them is due for route on Thursday.

### Schedule assignment belongs to this plan

Plan 03 owns a schedule's **shape**: which trips, in which order, with turnaround gaps cleared and empty movements accounted for. This plan owns the **physical train** that works it, and the validation that the assignment is possible.

Plan 03's Schedule record does not currently carry a train reference, though it defines a schedule as trips worked by one physical train. That reference is added, and the split is named in both documents.

Assignment validation checks:

- the assigned train's type can work every trip in the schedule — traction against the route's electrification, and length against every platform and every planned passing loop;
- the train is where the schedule starts, reached by a valid empty movement if it is not;
- the schedule's reversal gaps clear the type's minimum;
- the schedule leaves the train stabled somewhere it fits at the end of the week, and the week wraps consistently with plan 03's seven explicit days;
- the train's service interval is not overrun within the published timetable without a stay long enough to service it;
- no train is assigned two schedules that overlap in time.

A schedule with no train assigned is not an error while the draft is being edited. It is a **reported fleet requirement**: plan 03 already promises the player a fleet requirement before any trains are owned, and an unassigned schedule is exactly that promise — this week needs one more X61. Publication reports unassigned schedules, and their trips do not depart.

Automatic assignment is not part of v1. The data this plan defines is deliberately sufficient for a later solver — a schedule's requirements are explicit constraints, not implied by an editor — but nothing in v1 fills them in.
...
2. Open the catalogue, compare types by capacity, length, speed, and traction against the routes intended, and buy a train, choosing the depot it is delivered to.
3. Chain trips into schedules in the timetable workflow (plan 03), reading the reported fleet requirement where schedules have no train.
4. Assign a physical train to a schedule, and couple units where one is not enough.
5. Read assignment failures as named quantities — the length that does not fit, the wiring that is missing, the minutes the reversal is short, the route overrun on Thursday.
6. Fix by buying a train, rebuilding or wiring a depot, re-chaining the schedule, or placing a servicing stay in it.
7. Publish, and watch empty movements and depot arrivals on the map and the time-distance diagram alongside revenue trips.

The catalogue browser, fleet panel, depot panel, and how a schedule presents its assignment belong to plan 07.

## Minimum data and interface changes

Planning needs implied by the agreed behaviour, not settled schemas:

- **Train type:** identity, display name, role, traction, car count, length, maximum speed, acceleration, route braking rate, seated capacity and berths, maximum coupled units, minimum reversal time, service interval, and service duration. Static catalogue content, not player-editable.
- **Train:** identity, type, accumulated distance since last route, current location, and stabled siding. Individually identifiable.
- **Coupling:** the units forming one train for one schedule, constrained to a single type and the type's maximum.
...
- **Schedule:** a train reference, added to plan 03's record, with the ownership split named in both documents.
- **Empty movement:** owning schedule, endpoints, derived times, adjustability within its gap, and the occupancy it holds.
- **Servicing:** required visits over the published timetable, the stays that satisfy them, and overruns that block a next departure.
- **Validation:** assignment failures tied to the schedule and train that caused them, each naming the quantity that fails.
- **Interfaces consumed:** technical minimum running time, track, platform and loop occupancy, holding lengths, and strict electrification from plan 02; schedule shape, turnaround gaps, and the published timetable from plan 03.
...
- **Servicing location:** whether any depot can route any type, or whether servicing capability is a depot property the player builds and pays for.
...
- **Fleet requirement reporting:** how an unassigned schedule's requirement is expressed — a type, a capability, or a capacity — before any train exists to satisfy it.
- **Coupling granularity:** whether coupling is truly fixed for a whole schedule, or whether a peak-only strengthening schedule is common enough to need mid-schedule joining sooner than deferral assumes.

## Dependencies

- **Infrastructure and operations (02):** consumes this plan's length, performance, and traction for movement, separation, and electrification; supplies running times, occupancy, holding lengths, and the blocked-departure rule this plan follows for route overruns. Plan 02's separation formulae take this plan's braking rates as input.
- **Routes and timetabling (03):** supplies schedule shape, turnaround gaps, and the published timetable; consumes this plan's reversal minimum, length, traction, and capacity, and gains a train reference on its Schedule record. Publication there now checks newly published schedules against where trains actually are, so a schedule demanding a unit still working an old trip is reported at publication and, if published anyway, held at runtime with the blocking train named. The validation is this plan's; the reporting moment is 03's.
...
- **Interface and player experience (07):** now written. The catalogue, owned trains, depots and sidings live in a near-full-screen Fleet workspace, which is also where rolling stock is assigned to schedules — this plan's *schedules*, renamed there. Usable siding length is shown against drawn length as this plan requires. Named assignment failures surface both inline in Fleet and in the persistent bottom-bar status region.
...
- **First playable and validation (09):** now written. The corridor scenario uses two train types, one depot, and **only as many trains as the schedules require** — plan 08's ten-train corridor figure stays a separate stress fixture. A servicing visit is in scope, including the empty movement to a depot this plan now permits, and a train due away from its depot must never be stranded.
...
| Player couples two X61s for a schedule | The train's length and capacity are the sum; its acceleration, braking, and maximum speed are the single unit's; the coupling holds for the whole schedule. |
| Player tries to couple an X61 to an X40 | Refused: only units of the same type couple. |
| Player assigns an X2 to a schedule calling at a station with shorter platforms | The schedule is valid and the stop happens, with plan 02's overhang dwell penalty; length does not invalidate a platform stop. |
| Player assigns an X2 to a schedule whose planned meet is at a loop shorter than the train | The assignment is invalid before publication, naming the loop and the holding length, rather than failing at runtime. |
| Player assigns an electric unit to a schedule stabling at an unwired depot | Validation reports it before publication, per plan 02's electrification scenario. |
| A schedule's consecutive trips leave less than the type's minimum reversal time | The schedule is flagged with the shortfall as a named quantity, before publication. |
| A schedule requires a train to reach its first trip from a depot | An empty movement is derived, timed by the same running-time function, occupying real track and visible to the player. |
...
| A train comes due for route at a station with no depot | It may run empty to a depot that can service it. Every other departure is blocked until it has been. |
...
| A train reaches its service interval mid-week with no long enough stay in its schedule | Validation reports the overrun and the day it falls on; the player adds a servicing stay or re-chains. |
...
| A schedule has no train assigned | It is not an error in the draft; it is reported as a fleet requirement, and publication reports that its trips will not depart. |
| Two schedules overlapping in time are assigned the same train | Invalid, naming both schedules and the overlap. |
...
- No dynamic coupling or splitting partway through a schedule, and no mixed-type coupling.
