...
- **Interface and player experience (07):** now written. Construction keeps its contextual sidebar over the map rather than becoming a workspace, and gains the recurring-commitment figure alongside its existing preview of affected buildings and blocked roads. Named delay causes surface in a persistent bottom-bar status region. The time-distance diagram's scope question this plan left open is settled there: **one route or corridor at a time**, showing every trip that touches it. Assisted passing advice is presented as a before-and-after diff in operating terms, never exposing signalling.
...
- **First playable and validation (09):** now written. Milestone 1 proves one train runs, stops, reverses and reaches a depot with its actual running time matching the analytical minimum; milestone 2 exercises this plan's separation, contention and deadlock rules on a single-track section with a passing loop, where an express overtakes a regional and a late train's delay propagates with its cause named.
