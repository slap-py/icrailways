...
- ~~**First playable corridor**~~ — resolved in [plan 09](09-first-playable-and-validation.md). The corridor is kept: four stations, two routes for the regional and the express, one single-track section, a passing loop, one depot, two train types, and only as many trains as the schedules require. The express skips a station the regional serves, so that at least one transfer is genuinely useful rather than incidental. Delivery is six milestones, each defined by its exit evidence rather than by its code, and **the first playable is milestone 3, when passengers ride** — fares arrive there, so a week is comparable before the ledger exists. Financial consequences follow at milestone 4.
